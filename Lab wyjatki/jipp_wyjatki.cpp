...
class ilosc_wierszy : public exception
{
    virtual const char *what() const throw()
    {
        return "Rozna ilosc elementow w wierszach";
    }
};
class ilosc_kolumn : public exception
{
    virtual const char *what() const throw()
    {
        return "Rozna ilosc elementow w kolumnach";
    }
};
class kol_wier : public exception
{
    virtual const char *what() const throw()
    {
        return "Liczba kolumn pierwszej macierzy jest inna od liczby wierszy drugiej macierzy";
    }
};
class wier_kol : public exception
{
    virtual const char *what() const throw()
    {
        return "Liczba wierszy pierwszej macierzy jest inna od liczby kolumn drugiej macierzy";
    }
};
class blad_pliku : public exception
{
    virtual const char *what() const throw()
    {
...
Matrix Matrix::add(Matrix m2){
    ...
    ilosc_kolumn ik;
    try {
        if(N != m2.rows()) throw iw;
        if(M != m2.cols()) throw ik;
    Matrix wynik(N,M);
    for (int i=0;i<N;i++){
        for (int j=0;j<M;j++){
...
Matrix Matrix::multiply(Matrix m2)
{
    ...
    wier_kol w_k;
	try {
        if(N != m2.cols()) throw w_k;           
        if(M != m2.rows()) throw k_w;
